...
function initOne(id, rawOption) {
  ...
  // Turns off the CSS shimmer (see styles/skeleton.css) now that this
  // chart has real content -- driven by the actual setOption() call
  // completing, not a timer, so it never lingers past the real wait.
...
export function init() {
  if (!window.echarts) {
    ...
    document.querySelectorAll('.echarts-container').forEach(function (el) {
      el.classList.add('is-hydrated'); // stop shimmering on nothing
    });
    return;
  }
...
export function resizeWithin(cardEl) {
  if (!cardEl) return;
