...
52. **Renamed the app's display branding from "StockLLM" to "ADELE"**
    (0.9.29) — user's backronym, arrived at over several rounds of
    naming discussion: **A**gent-**D**riven **E**valuation **L**LM
    **E**ngine, named after his wife. Scoped deliberately narrow, per
    the user's own words ("update it in dashboard"): every user-facing
    "StockLLM" string across the dashboard and webapp UI (page
    `<title>`s, the `apple-mobile-web-app-title` meta tag, the footer
    disclaimer, the "AI Recommendation" glossary tooltip text, the
    "Download for AI Chat" export's own header/footer text, both the
    login and ticker-form page headings) plus `config.yaml`'s display
    `name:` field. Also updated code comments/docstrings that named the
    app, for consistency, since leaving them stale after an intentional
    rename would just be new tech debt.
    - **Deliberately NOT touched**: `config.yaml`'s `slug: stockllm` (HA
      Supervisor's actual identity for the installed add-on -- changing
      it means uninstall/reinstall, not a rename, exactly the risk
      flagged when this was first discussed), the GitHub repo name, and
      the on-disk repo folder name (`/share/StockLLM/` -- `tests/
      test_config.py`'s `OUTPUT_DIR` test checks for this exact path
      component, which is a real filesystem detail, not app branding --
      confirmed by reading `config.py`'s `OUTPUT_DIR` default before
      touching that test, and left it alone). The user said he'd handle
      pointing `adele.baksht.org` at the existing instance himself --
      doesn't require any of the above to change on this end.
    - Test assertions on the literal string broke as expected (`"StockLLM"
      in resp.data`, the apple-mobile-web-app-title meta tag, the export
      round-trip's `"Generated by StockLLM on ..."` regex) -- updated
      all of them to `"ADELE"` alongside the source changes, in
      `tests/test_webapp.py`, `tests/test_dashboard_build.py`, and
      `tests/test_llm_export.py`.
    - **Verified for real**: full pytest suite green (480 passed) after
      fixing the assertions above; grepped the whole `dashboard/`/
